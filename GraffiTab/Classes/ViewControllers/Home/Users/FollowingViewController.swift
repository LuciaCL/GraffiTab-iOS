...
class FollowingViewController: ListUsersViewController {
    ...
    override func loadItems(isStart: Bool, offset: Int, cacheBlock: (response: GTResponseObject) -> Void, successBlock: (response: GTResponseObject) -> Void, failureBlock: (response: GTResponseObject) -> Void) {
        GTUserManager.getFollowing(user!.id!, offset: offset, cacheResponse: isStart, cacheBlock: cacheBlock, successBlock: successBlock, failureBlock: failureBlock)
    }
    ...
    override func setupTopBar() {
        ...
        self.title = NSLocalizedString("controller_following", comment: "")
    }
}
