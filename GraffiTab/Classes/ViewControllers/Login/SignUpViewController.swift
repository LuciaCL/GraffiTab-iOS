        ...
        if InputValidator.validateSignUp(fn!, lastName: ln!, email: e!, username: u!, password: p!, confirmPassword: cp!) {
            ...
            GTUserManager.register(fn!, lastName: ln!, email: e!, username: u!, password: p!, successBlock: { (response) in
                ...
                if (response.reason == .BadRequest) {
                    DialogBuilder.showErrorAlert("We have found another user that matches these details. Please consider choosing a different email address or username.", title: App.Title)
                    return
                }
