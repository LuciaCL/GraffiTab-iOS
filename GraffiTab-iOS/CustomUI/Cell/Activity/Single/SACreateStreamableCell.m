...
@interface SACreateStreamableCell () {
    
    GTActivityCreateStreamable *typedItem;
}

@end

@implementation SACreateStreamableCell
...
- (void)setItem:(GTActivityContainer *)item {
    ...
    typedItem = (GTActivityCreateStreamable *)item.activities.firstObject;
    
    NSString *text = [NSString stringWithFormat:NSLocalizedString(@"SA_CREATE", nil), item.activityUser.fullName];
