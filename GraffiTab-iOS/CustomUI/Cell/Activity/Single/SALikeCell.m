...
@interface SALikeCell () {
    
    GTActivityLike *typedItem;
}

@end

@implementation SALikeCell
...
- (void)setItem:(GTActivityContainer *)item {
    ...
    typedItem = (GTActivityLike *)item.activities.firstObject;
    
    NSString *text = [NSString stringWithFormat:NSLocalizedString(@"SA_LIKE", nil), item.activityUser.fullName];
