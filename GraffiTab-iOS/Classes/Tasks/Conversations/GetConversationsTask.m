...
@implementation GetConversationsTask
...
- (void)getConversationsWithStart:(int)start numberOfItems:(int)count successBlock:(void (^)(ResponseObject *))successBlock cacheBlock:(void (^)(ResponseObject *))cacheBlock failureBlock:(void (^)(ResponseObject *))failureBlock {
    ...
    NSString *string = [RequestBuilder buildGetConversations];
...
- (id)parseJsonSuccessObject:(NSDictionary *)json {
    ...
    for (NSDictionary *personJson in jsonArray) {
        Conversation *p = [[Conversation alloc] initFromJson:personJson];
        [items addObject:p];
    }
